...
# 2. 关闭系统时间同步，避免 CLOCK_REALTIME 被回拨后带动 PTP 时间戳倒退
echo "🕒 正在关闭系统 NTP/time sync 服务，避免运行中回调系统时间..."
timedatectl set-ntp false 2>/dev/null || true
systemctl stop systemd-timesyncd.service 2>/dev/null || true
systemctl stop chrony.service 2>/dev/null || true

TIMESYNCD_STATE=$(systemctl is-active systemd-timesyncd.service 2>/dev/null || true)
CHRONY_STATE=$(systemctl is-active chrony.service 2>/dev/null || true)
echo "   systemd-timesyncd: ${TIMESYNCD_STATE:-unknown}"
echo "   chrony: ${CHRONY_STATE:-unknown}"
