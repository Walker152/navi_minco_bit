...
public:
  ...
  std::string formatPoint(const geometry_msgs::msg::Point& point, int precision = 2) {
    std::ostringstream ss;
    ss << "(" << formatFloat(point.x, precision) << ", " 
       << formatFloat(point.y, precision) << ", " 
  ...
  std::string formatPose(const geometry_msgs::msg::Pose& pose, int precision = 2) {
    std::ostringstream ss;
    ss << "位置: " << formatPoint(pose.position, precision)
       << ", 朝向: (" << formatFloat(pose.orientation.x, precision) << ", "
       << formatFloat(pose.orientation.y, precision) << ", "
  ...
  BT::NodeStatus tick() override
  {
    ...
    auto printBlackboardVariables = [this, blackboard]() -> BT::NodeStatus
    {
      ...
      try
      {
        ...
        } catch(...) { std::cout << "  ▸ 游戏剩余时间: 不可用" << std::endl; }

        try {
          auto game_status = blackboard->get<int>("game_status");
          std::string status_str = "未知";
          switch(game_status) {
            case 0: status_str = "未开始"; break;
            case 1: status_str = "进行中"; break;
            case 2: status_str = "暂停"; break;
            case 3: status_str = "结束"; break;
          }
          std::cout << "  ▸ 游戏状态: " << status_str << " (" << game_status << ")" << std::endl;
        } catch(...) { std::cout << "  ▸ 游戏状态: 不可用" << std::endl; }
        std::cout << std::endl;
          ...
          for (const auto& ally : allies_info) {
            if (ally.robot_id ==1) {
              std::cout << "  ▸ \033[1;33m【英雄】\033[0m ID: " << ally.robot_id 
                        << ", 血量: " << ally.remain_hp 
                        << ", 位置: " << formatPoint(ally.position) << std::endl;
              hero_found = true;
              break;
            }
          }
        ...
        } catch(...) { std::cout << "  ▸ 敌方前哨站状态: 不可用" << std::endl; }
        std::cout << std::endl;
        ...
        try {
          ...
          std::cout << "  ▸ 热量: " << current_heat << "/" << heat_limit 
                    << ", 冷却值: " << cooling_value 
                    << " (" << std::fixed << std::setprecision(1) 
                    << (heat_limit > 0 ? (static_cast<float>(current_heat) / heat_limit * 100) : 0) 
                    << "%)" << std::endl;
        } catch(...) { std::cout << "  ▸ 热量信息: 不可用" << std::endl; }
        ...
        } catch(...) { std::cout << "  ▸ 目标有效性: 不可用" << std::endl; }
        ...
        if (blackboard->get<bool>("target_valid")) {
          ...
          } catch(...) { std::cout << "  ▸ 目标装甲板ID: 不可用" << std::endl; }
          ...
          } catch(...) { std::cout << "  ▸ 目标位置: 不可用" << std::endl; }
        } else {
        ...
        } catch(...) { std::cout << "  ▸ 免费复活: 不可用" << std::endl; }
        ...
        try {
          ...
          std::cout << std::endl;
        } catch(...) { std::cout << "  ▸ 立即复活: 不可用" << std::endl; }
        std::cout << std::endl;
        ...
        } catch(...) { std::cout << "  ▸ 云台偏航角: 不可用" << std::endl; }
        ...
        try {
          ...
          std::cout << "  ▸ 事件码: 0x" << std::hex << std::uppercase << event_code_raw << std::dec << std::endl;
        } catch(...) { std::cout << "  ▸ 事件码: 不可用" << std::endl; }

        try {
          auto sentry_info_1_raw = blackboard->get<uint32_t>("sentry_info_1_raw");
          std::cout << "  ▸ 哨兵信息1: 0x" << std::hex << std::uppercase << sentry_info_1_raw << std::dec << std::endl;
        } catch(...) { std::cout << "  ▸ 哨兵信息1: 不可用" << std::endl; }

        try {
          auto sentry_info_2_raw = blackboard->get<uint16_t>("sentry_info_2_raw");
          std::cout << "  ▸ 哨兵信息2: 0x" << std::hex << std::uppercase << sentry_info_2_raw << std::dec << std::endl;
        } catch(...) { std::cout << "  ▸ 哨兵信息2: 不可用" << std::endl; }
