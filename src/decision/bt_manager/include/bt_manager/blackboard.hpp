...
public:
  Blackboard()
  {
    ...
    blackboard_->set<uint8_t>("planned_tunnel_yaw_align_mode", 0);                 // 局部轨迹判定的隧道航向
    ...
    blackboard_->set<int>("enhanced_attack_remaining_time", 15);   // 强化攻击剩余可用时间(秒),每局上限15s
    blackboard_->set<int>("enhanced_defend_remaining_time", 15);   // 强化防御剩余可用时间(秒),每局上限15s
    blackboard_->set<int>("enhanced_move_remaining_time", 15);     // 强化移动剩余可用时间(秒),每局上限15s
