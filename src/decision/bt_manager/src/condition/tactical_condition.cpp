...
BT::NodeStatus CheckAttackCondition::tick()
{
  ...
  try {
    ...
    const bool can_activate_energy = blackboard->get<bool>("can_activate_energy");
    ...
    const bool energy_active = can_activate_energy || small_energy_status > 0 || big_energy_status > 0;
