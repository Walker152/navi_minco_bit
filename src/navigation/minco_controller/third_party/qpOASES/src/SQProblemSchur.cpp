 ...
 *	\author Andreas Waechter and Dennis Janka, based on QProblem.cpp by Hans Joachim Ferreau, Andreas
 *Potschka, Christian Kirches \version 3.2 \date 2012-2017
 *
  ...
  const real_t * ubA_new)
{
  ...
  if ((getStatus() == QPS_NOTINITIALISED) || (getStatus() == QPS_PREPARINGAUXILIARYQP) ||
  ...
  if (H_new != 0) {
    ...
    if ((hessianType == HST_ZERO) || (hessianType == HST_SEMIDEF) || (usingRegularisation() == BT_TRUE)) {
  ...
  if (returnvalue == SUCCESSFUL_RETURN && neig == getNAC()) {
    ...
    /* b) KKT matrix has too many negative eigenvalues. Try to correct the inertia by adding bounds (reduce
     * nullspace dimension). */
    if (options.printLevel == PL_HIGH)
      MyPrintf("WARNING: In hotstart for new matrices, reduced Hessian for initial working set has %i "
               "negative eigenvalues, should be %i.\n",
        neig,
        getNAC());
  ...
  BooleanType setupAfresh)
{
  ...
  /* III.2.) Check if inertia is correct. If so, we now have a linearly independent working set with a pos
   * def reduced Hessian */
  int_t neig = sparseSolver->getNegativeEigenvalues();
  if (neig == getNAC()) {
  ...
  if (options.printLevel == PL_HIGH)
    MyPrintf("WARNING: In setupAuxiliaryWorkingSet: Initial working set reduced Hessian has %i negative "
 ...
 */
returnValue SQProblemSchur::addConstraint(
  int_t number, SubjectToStatus C_status, BooleanType updateCholesky, BooleanType ensureLI)
{
  int_t idxDeleted = -1;
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      returnValue returnvalue = addToSchurComplement(number,
        SUT_ConAdded,
        numNonzerosM,
        MNpos,
        MNvals,
...
returnValue SQProblemSchur::addConstraint_checkLISchur(int_t number, real_t * xiC, real_t * xiB)
{
  ...
  /* For the Schur complement version we only use options.enableFullLITests = TRUE */
  {
    ...
    returnValue dsdreturnvalue = determineStepDirection(
      delta_g, nul, nul, nul, nul, BT_FALSE, BT_FALSE, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
...
returnValue SQProblemSchur::addConstraint_ensureLI(int_t number, SubjectToStatus C_status)
{
  ...
  performRatioTest(
    nAC, AC_idx, &constraints, num, xiC, options.epsNum, options.epsDen, y_min, y_min_number);
  ...
  performRatioTest(
    nFX, FX_idx, &bounds, num, xiB, options.epsNum, options.epsDen, y_min, y_min_number_bound);
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound == BT_TRUE) {
      ...
      getGlobalMessageHandler()->throwInfo(
        RET_REMOVE_FROM_ACTIVESET, messageString, __FUNCTION__, __FILE__, __LINE__, VS_VISIBLE);
#endif
 ...
 *	a d d B o u n d
 */
returnValue SQProblemSchur::addBound(
  int_t number, SubjectToStatus B_status, BooleanType updateCholesky, BooleanType ensureLI)
{
  int_t idxDeleted = -1;
 ...
 */
returnValue SQProblemSchur::removeConstraint(
  int_t number, BooleanType updateCholesky, BooleanType allowFlipping, BooleanType ensureNZC)
{
  returnValue returnvalue = SUCCESSFUL_RETURN;
  ...
  if (ensureNZC == BT_TRUE) {
    returnvalue =
      ensureNonzeroCurvature(BT_FALSE, number, exchangeHappened, addBoundNotConstraint, addIdx, addStatus);
    ...
    if (nS < 0 || nS == nSmax) {
      ...
      for (int_t i = 0; i < nACStart; i++)
        if (AC_idxStart[i] == number) {
          ...
          if (addToSchurComplement(number, SUT_ConRemoved, 1, &pos, &one, 0, 0, 0, 0.0) !=
              SUCCESSFUL_RETURN)
            return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
          found = true;
          break;
  ...
  if ((options.enableFlippingBounds == BT_TRUE) && (allowFlipping == BT_TRUE) &&
      (exchangeHappened == BT_FALSE)) {
    if (sModType == 1) { /* Case 1: We added a row and column to S. */
      ...
      } else { /* Determinants have the correct sign, compute QR of new (larger) S */
        updateSchurQR(idxDeleted);
      }
    } else if (sModType == 2) { /* Case 2: We deleted a row and column of S. */
      ...
      } else { /* Determinants have the correct sign, compute QR of new (smaller) S */
        updateSchurQR(idxDeleted);
      }
    } else if (sModType == 3) { /* Case 3: S was reset. */
      ...
      if (correctInertia())
        return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
    } else { /* None of the three cases happened */
      return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
    }
  } else { /* No flipping strategy, update QR factorization of S */
    updateSchurQR(idxDeleted);
  }
 ...
 */
returnValue SQProblemSchur::removeBound(
  int_t number, BooleanType updateCholesky, BooleanType allowFlipping, BooleanType ensureNZC)
{
  returnValue returnvalue = SUCCESSFUL_RETURN;
  int_t addIdx;
  ...
  if (ensureNZC == BT_TRUE) {
    returnvalue =
      ensureNonzeroCurvature(BT_TRUE, number, exchangeHappened, addBoundNotConstraint, addIdx, addStatus);
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      returnvalue = addToSchurComplement(
        number, SUT_VarFreed, numNonzerosM, Mpos, Mvals, numNonzerosN, Npos, Nvals, N_diag);
 ...
 *	s e t u p T Q f a c t o r i s a t i o n
 */
returnValue SQProblemSchur::backsolveT(
  const real_t * const b, BooleanType transposed, real_t * const a) const
{
  return THROWERROR(RET_UNKNOWN_BUG);
}
 ...
 *	b a c k s o l v e R
 */
returnValue SQProblemSchur::backsolveR(
  const real_t * const b, BooleanType transposed, real_t * const a) const
{
  return THROWERROR(RET_UNKNOWN_BUG);
}
 ...
 */
returnValue SQProblemSchur::backsolveR(
  const real_t * const b, BooleanType transposed, BooleanType removingBound, real_t * const a) const
{
  return THROWERROR(RET_UNKNOWN_BUG);
}
...
real_t SQProblemSchur::calcDetSchur(int_t idxDel)
{
  ...
  /* Case 2: row and column idxDel have been deleted from S */
    ...
    for (i = idxDel; i < nS; i++) {
      computeGivens(
        tempR[i + i * dim], tempR[(i + 1) + i * dim], tempR[i + i * dim], tempR[(i + 1) + i * dim], c, s);
      nu = s / (1.0 + c);
    ...
    /* Delete row: nS Givens rotations to transform last column (and row!) of (old) Q**T to getAbs((nS+1)-th
     * unity vector) */
    for (i = nS; i > 0; i--) {
...
returnValue SQProblemSchur::updateSchurQR(int_t idxDel)
{
  ...
  if (idxDel < 0) {
    ...
    for (i = 0; i < nS - 1; i++) {
      computeGivens(
        R_[i + i * nSmax], R_[(nS - 1) + i * nSmax], R_[i + i * nSmax], R_[(nS - 1) + i * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = i + 1; j < nS; j++)
        applyGivens(c,
          s,
  ...
  /* Case 2: row and column idxDel have been deleted from S */
    ...
    for (i = idxDel; i < nS; i++) {
      computeGivens(
        R_[i + i * nSmax], R_[(i + 1) + i * nSmax], R_[i + i * nSmax], R_[(i + 1) + i * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = i + 1; j < nS; j++)
        applyGivens(
          c, s, nu, R_[i + j * nSmax], R_[(i + 1) + j * nSmax], R_[i + j * nSmax], R_[(i + 1) + j * nSmax]);

      /* Simultaneously transform (old) Q**T (coldim is one larger)*/
      for (j = 0; j < nS + 1; j++)
        applyGivens(
          c, s, nu, Q_[j + i * nSmax], Q_[j + (i + 1) * nSmax], Q_[j + i * nSmax], Q_[j + (i + 1) * nSmax]);
    }
    ...
    /* IV: Delete row: nS Givens rotations to transform last column (and row!) of (old) Q**T to
     * getAbs((nS+1)-th unity vector) */
    for (i = nS; i > 0; i--) {
      computeGivens(
        Q_[nS + nS * nSmax], Q_[nS + (i - 1) * nSmax], Q_[nS + nS * nSmax], Q_[nS + (i - 1) * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = 0; j < nS; j++)
        applyGivens(c,
          s,
 ...
 */
returnValue SQProblemSchur::backsolveSchurQR(
  int_t dimS, const real_t * const rhs, int_t dimRhs, real_t * const sol)
{
  if (dimS < 1 || dimRhs < 1)
    return SUCCESSFUL_RETURN;
  ...
  if (INFO != 0) {
    ...
    if (INFO == ((long)0xDEADBEEF))
      MyPrintf("If SQProblemSchur is to be used, system LAPACK must be used instead of the qpOASES LAPACK "
               "replacement");
    return RET_QR_FACTORISATION_FAILED;
  }
...
returnValue SQProblemSchur::determineStepDirection(const real_t * const delta_g,
  ...
  real_t * const delta_yFX)
{
  returnValue retval = determineStepDirection2(delta_g,
    delta_lbA,
    delta_ubA,
    delta_lb,
    delta_ub,
  ...
  if (retval == RET_QR_FACTORISATION_FAILED) {
    ...
    retval = determineStepDirection2(delta_g,
      delta_lbA,
      delta_ubA,
      delta_lb,
      delta_ub,
  ...
  real_t * const delta_yFX)
{
  /* The linear system to be solved here is this:
     ...
     | A_CF  A_CX    0     0 | | -delta_yAC | = |  delta_bA  |  <-- active entries of delta_lbA and
     delta_ubA with corresponding sign \  0     I      0     0 / \ -delta_yFX /   \  delta_bX  /  <-- fixed
     entries of delta_lb and delta_ub with corresponding sign
...
returnValue SQProblemSchur::resetSchurComplement(BooleanType allowInertiaCorrection)
{
  ...
  A->getSparseSubmatrix(
    constraints.getActive(), bounds.getFree(), nFR + 1, 1, numNonzerosA, 0, 0, 0, BT_FALSE);
  ...
  A->getSparseSubmatrix(constraints.getActive(),
    bounds.getFree(),
    nFR + 1,
    1,
    numNonzerosA,
...
returnValue SQProblemSchur::computeMTimes(
  real_t alpha, const real_t * const x_, real_t beta, real_t * const y_)
{
...
returnValue SQProblemSchur::computeMTransTimes(
  real_t alpha, const real_t * const x_, real_t beta, real_t * const y_)
{
...
returnValue SQProblemSchur::addToSchurComplement(int_t number,
  ...
  int_t numNonzerosM,
  const sparse_int_t * Mpos,
  const real_t * const Mvals,
  int_t numNonzerosN,
...
{
  if (options.printLevel == PL_HIGH)
    MyPrintf("deleting entry %d with idx = %d and type %d from Schur complement.",
      idx,
      schurUpdateIndex[idx],
      schurUpdate[idx]);
