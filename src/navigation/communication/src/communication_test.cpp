...
int main(int argc, char ** argv)
{
  ...
  auto nav_sub = test_node->create_subscription<ros_interfaces::msg::Nav>(
    "/NavRequest", 10, [&state](ros_interfaces::msg::Nav::ConstSharedPtr msg) {
      state.nav_received = true;
      state.last_nav = *msg;
    });
  auto evt_sub = test_node->create_subscription<ros_interfaces::msg::EventStatus>(
    "/sentry/event_status", 10, [&state](ros_interfaces::msg::EventStatus::ConstSharedPtr msg) {
      state.evt_received = true;
      state.last_evt = *msg;
    });
  ...
  cmd.linear.x = 0.6;
  cmd.linear.y = 0.2;
  cmd.angular.z = 0.0;  // 触发非零速度
  nav_msgs::msg::Odometry odom;
  odom.pose.pose.orientation.w = 1.0;  // 零姿态
