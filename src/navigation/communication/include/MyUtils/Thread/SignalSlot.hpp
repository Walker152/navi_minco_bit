...
template <typename Callback> struct SlotImpl
{
  ...
  SlotImpl(const std::shared_ptr<Data> & data, Callback && cb) : data_(data), cb_(cb), tie_(), tied_(false)
  {
  }
...
public:
  ...
  {
    std::shared_ptr<SlotImpl_t> slotImpl(new SlotImpl_t(impl_, std::forward<Callback>(func)));
    add(slotImpl);
    return slotImpl;
  }
