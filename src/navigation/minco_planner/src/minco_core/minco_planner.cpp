  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // Odometry topic (for speed query during emergency stop blocking).
  std::string odom_topic{"/odom"};
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".odom_topic", rclcpp::ParameterValue(odom_topic));
  node->get_parameter(name + ".odom_topic", odom_topic);
...
bool MincoPlanner::ReplanLocal(const geometry_msgs::msg::PoseStamped & current_pose)
{
  ...
  std::vector<Eigen::Vector3d> sparse_path = utils::getSparseWaypoints(
    ...
    [this](const Eigen::Vector3d & a, const Eigen::Vector3d & b) {return this->isLineFree(a, b);} );
