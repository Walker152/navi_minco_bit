...
/**
 * @struct minco_planner::smac::SearchInfo
...
/**
 * @struct minco_planner::smac::SmootherParams
...
struct SmootherParams
{
   ...
   */
  void get(rclcpp_lifecycle::LifecycleNode::SharedPtr node, const std::string & name)
  {
    std::string local_name = name + std::string(".smoother.");

    // Smoother params
    tolerance_ = node->declare_parameter(local_name + "tolerance", 1e-10);
    max_its_ = node->declare_parameter(local_name + "max_iterations", 1000);
    w_data_ = node->declare_parameter(local_name + "w_data", 0.2);
    w_smooth_ = node->declare_parameter(local_name + "w_smooth", 0.3);
    do_refinement_ = node->declare_parameter(local_name + "do_refinement", true);
    refinement_num_ = node->declare_parameter(local_name + "refinement_num", 2);
  }
...
/**
 * @struct minco_planner::smac::TurnDirection
...
/**
 * @struct minco_planner::smac::MotionPose
 * @brief A struct for poses in motion primitives
 */
struct MotionPose
{
  /**
   * @brief A constructor for minco_planner::smac::MotionPose
   */
  MotionPose() {}
...
/**
 * @struct minco_planner::smac::LatticeMetadata
...
/**
 * @struct minco_planner::smac::MotionPrimitive
...
#endif  // MINCO_PLANNER__SMAC_SEARCH__TYPES_HPP_
