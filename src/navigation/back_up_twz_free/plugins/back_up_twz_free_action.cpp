...
void BackUpTwzFree::onConfigure()
{
  ...
  nav2_util::declare_parameter_if_not_declared(node, robot_radius_param, rclcpp::ParameterValue(0.1));
  node->get_parameter(robot_radius_param, robot_radius_);

  nav2_util::declare_parameter_if_not_declared(node, max_radius_param, rclcpp::ParameterValue(1.0));
  node->get_parameter(max_radius_param, max_radius_);

  if (max_radius_ < robot_radius_) {
    RCLCPP_WARN(
      node->get_logger(), "max_radius is smaller than robot_radius. Setting max_radius to robot_radius");
    max_radius_ = robot_radius_;
  }

  nav2_util::declare_parameter_if_not_declared(
    node, service_name_param, rclcpp::ParameterValue(std::string("local_costmap/get_costmap")));
  node->get_parameter(service_name_param, service_name_);

  nav2_util::declare_parameter_if_not_declared(node, free_threshold_param, rclcpp::ParameterValue(5));
  node->get_parameter(free_threshold_param, free_threshold_);

  nav2_util::declare_parameter_if_not_declared(node, cost_threshold_param, rclcpp::ParameterValue(0.1));
  node->get_parameter(cost_threshold_param, cost_threshold_);

  nav2_util::declare_parameter_if_not_declared(node, visualization_param, rclcpp::ParameterValue(false));
  node->get_parameter(visualization_param, visualization_);

  nav2_util::declare_parameter_if_not_declared(
    node, local_costmap_global_frame_param, rclcpp::ParameterValue(std::string("camera_init")));
  node->get_parameter(local_costmap_global_frame_param, local_costmap_global_frame_);

  nav2_util::declare_parameter_if_not_declared(
    node, local_costmap_robot_base_frame_param, rclcpp::ParameterValue(std::string("base_link")));
...
Status BackUpTwzFree::onRun(const std::shared_ptr<const BackUpAction::Goal> command)
{
  ...
  if (!nav2_util::getCurrentPose(initial_pose_,
        *tf_,
        local_costmap_global_frame_,
        local_costmap_robot_base_frame_,
  ...
  while (!free_space_found && radius <= max_radius_) {
    ...
    for (auto i = 0; i < costmap.metadata.size_x; i++) {
      for (auto j = 0; j < costmap.metadata.size_y; j++) {
        // RCLCPP_INFO(node->get_logger(), "机器人坐标系: %s 全局坐标系: %s", robot_base_frame_.c_str(),
        // global_frame_.c_str()); RCLCPP_INFO(node->get_logger(), "方位差值: %lf, %lf",
        // costmap.metadata.origin.position.x - pose_x, costmap.metadata.origin.position.y - pose_y);
        auto costmap_index = i + j * costmap.metadata.size_x;
        ...
        // RCLCPP_INFO(node->get_logger(), "代价地图原点: (%lf, %lf) 当前机器人位置：(%lf, %lf)
        // 当前搜索点位置：(%lf, %lf) 当前距离：%lf 当前搜索半径：%lf 当前代价: %d",
        // costmap.metadata.origin.position.x, costmap.metadata.origin.position.y, pose_x, pose_y, x, y,
        // distance_to_center, radius, costmap.data[costmap_index]);
        if (distance_to_center <= radius) {
          // RCLCPP_INFO(node->get_logger(), "在半径中 代价值：%d", costmap.data[costmap_index]);
          if (costmap.data[costmap_index] <= cost_threshold_) {
            RCLCPP_INFO(node->get_logger(),
              "找到一个安全点 当前机器人位置：(%lf, %lf) 当前安全点位置：(%lf, %lf) 当前距离：%lf "
              "当前搜索半径：%lf",
              pose_x,
              pose_y,
              x,
  ...
  RCLCPP_WARN(this->logger_, "backing up %f meters towards free space at angle %f", command_x_, angle_diff);
