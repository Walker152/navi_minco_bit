...
namespace rog_map {
    ...
    // 3) 垂直扫描范围：用本地地图边界（与 SlidingMap 同步）
        int z_ground_id{}, z_ceil_id{};
        posToGlobalIndex(local_map_bound_min_d_.z(), z_ground_id);
        posToGlobalIndex(local_map_bound_max_d_.z(), z_ceil_id);
        if (z_ground_id > z_ceil_id) std::swap(z_ground_id, z_ceil_id);

        // 4) 对每个 (x,y) 自顶向下扫描，找“上方自由且下方有支撑（或在虚拟地面以下）”的最高 z
        for (int x = min_g.x(); x <= max_g.x(); ++x) {
            for (int y = min_g.y(); y <= max_g.y(); ++y) {
                ...
                for (int z = z_ceil_id; z >= z_ground_id; --z) {
                    ...
                    Vec3f pos;
                    globalIndexToPos(id_g, pos);
                    const bool free_here = map.isKnownFree(pos);
                    // 取下方一点作为支撑检测
                    const Vec3f pos_below(pos.x(), pos.y(), pos.z() - sc_.resolution);
                    const bool support_below = (z - 1 < z_ground_id) || map.isOccupied(pos_below);
                    if (free_here && support_below) {
