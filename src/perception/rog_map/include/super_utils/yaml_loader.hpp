...
private:
  ...
  template <typename T>
  bool loadParamInternal(
    const std::string & param_name, T & param_value, const T & default_value, bool required)
  {
      ...
      if constexpr (is_vector<T>::value) {
        if (tmp_node.Type() == YAML::NodeType::Sequence) {
          try {
            ...
            printf(
              "\033[0;31m Type mismatch for param %s, using default value: \033[0;0m", param_name.c_str());
  ...
  static bool containsSlash(const std::string & str) { return str.find('/') != std::string::npos; }

  template <typename T> void printValue(const T & param_value)
  {
    if constexpr (is_vector<T>::value) {
      std::cout << "[";
  ...
  [[nodiscard]] YAML::Node getNodeFromPath(const std::string & path) const
  {
    ...
    if (!node[path.substr(pos)].IsNull() && node[path.substr(pos)]) {
