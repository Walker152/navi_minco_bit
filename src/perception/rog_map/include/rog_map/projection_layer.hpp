...
private:
    ...
    void updateOneCell(size_t idx, int x, int y, const ProjectionLayerConfig &config, const ColumnScanner &scanner,
                       const std::vector<CellData> &previous);
    void updateTerrainNeighborhood(const ProjectionLayerConfig &config, const std::vector<uint8_t> *update_mask);
