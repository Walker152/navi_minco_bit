...
public:
  ...
  Config(const string & cfg_path, const string & name_space = "rog_map")
  {
    ...
    throw std::runtime_error(
      " -- [SlidingMap]: ORIGIN_AT_CORNER and ORIGIN_AT_CENTER cannot be both true!");
#endif
#endif
...
#ifndef ORIGIN_AT_CENTER
    throw std::runtime_error(
      " -- [SlidingMap]: ORIGIN_AT_CORNER and ORIGIN_AT_CENTER cannot be both false!");
#endif
#endif
    ...
    if (esdf_en) {
      if (temp_esdf_update_box.size() != 3) {
        ...
        esdf_local_update_box =
          Vec3f(temp_esdf_update_box[0], temp_esdf_update_box[1], temp_esdf_update_box[2]);
      }
    }
    ...
    if (load_pcd_en) {
      ...
      std::cout << BLUE << " -- [ROG-Config] pcd_name: " << pcd_name << RESET << std::endl;
    }
    ...
    if (resolution > inflation_resolution) {
      throw std::invalid_argument(
        "The inflation resolution should be equal or larger than the resolution!");
    }
    ...
    if (point_filt_num <= 0) {
      std::cout << color_text::YELLOW
                << " -- [ROG] point_filt_num should be larger or equal than 1, it is set to 1 now." << RESET
                << std::endl;
      point_filt_num = 1;
    }
    ...
    if (batch_update_size <= 0) {
      std::cout << color_text::YELLOW
                << " -- [ROG] batch_update_size should be larger or equal than 1, it is set to 1 now."
                << RESET << std::endl;
      batch_update_size = 1;
    }
    ...
    loader.LoadParam(
      name_space + "/raycasting/local_update_box", update_box, vector<double>{999, 999, 999});
    ...
    loader.LoadParam(name_space + "/relative_ceil_height", relative_ceil_height, 2.0);

    loader.LoadParam(name_space + "/elevation_filter_z_min", elevation_filter_z_min, -10.0);
    loader.LoadParam(name_space + "/elevation_filter_z_max", elevation_filter_z_max, 10.0);
...
#define logit(x) (log((x) / (1 - (x))))
    ...
    int z_inflation_step = (inflation_step > 0) ? 1 : 0;
    for (int dx = -inflation_step; dx <= inflation_step; dx++) {
      for (int dy = -inflation_step; dy <= inflation_step; dy++) {
        for (int dz = -z_inflation_step; dz <= z_inflation_step; dz++) {
          if (inflation_step == 1 || dx * dx + dy * dy <= inflation_step * inflation_step) {
    ...
    if (unk_inflation_en) {
      ...
      // init spherical neighbor
      int unk_z_inflation_step = (unk_inflation_step > 0) ? 1 : 0;
      for (int dx = -unk_inflation_step; dx <= unk_inflation_step; dx++) {
        for (int dy = -unk_inflation_step; dy <= unk_inflation_step; dy++) {
          for (int dz = -unk_z_inflation_step; dz <= unk_z_inflation_step; dz++) {
            if (unk_inflation_step == 1 || dx * dx + dy * dy <= unk_inflation_step * unk_inflation_step) {
  ...
  double relative_ceil_height{};
  double elevation_filter_z_min{}, elevation_filter_z_max{};
