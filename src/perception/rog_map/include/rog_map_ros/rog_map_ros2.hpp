...
class ROGMapROS : public ROGMap
{
  ...
  const double getSystemWalltimeNow() override { return nh_->get_clock()->now().seconds(); }

  void getSystemWalltimeNow(rclcpp::Time & _in) { _in = nh_->get_clock()->now(); };

  struct VisualizeMap
  {
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr occ_pub, unknown_pub, esdf_neg_pub,
      esdf_occ_pub, occ_inf_pub, unknown_inf_pub, frontier_pub, esdf_pub, elev_pub;
  ...
  void updateCallback()
  {
    ...
    if (rc_.unfinished_frame_cnt > 0) {
      ...
      // If no new frame, we still update the map (e.g. for sliding or ESDF update)
      // using the latest robot pose and an empty cloud.
      temp_pc.clear();
      temp_pose = std::make_pair(robot_state_.p, robot_state_.q);
    }
    rc_.updete_lock.unlock();
    ...
    if (elev_map_) {
      // Elevation map only needs data refresh here; sliding is unified in ProbMap
      Vec3f box_max = robot_state_.p + cfg_.local_update_box_d / 2;
      Vec3f box_min = robot_state_.p - cfg_.local_update_box_d / 2;
  ...
  void vizCallback()
  {
    ...
    if ((box_max - box_min).minCoeff() <= 0) {
      // Fallback to full local map range if the robot-centered box is invalid
      // std::cout << YELLOW << " -- [ROGMap] Visualization range is too small around robot. Fallback to
      // full local map." << RESET << std::endl;
      box_min = local_map_bound_min_d_;
      box_max = local_map_bound_max_d_;
    }
    ...
    if (vm_.occ_pub->get_subscription_count() >= 1) {
      ...
      RCLCPP_INFO(nh_->get_logger(), "Published occupied map with %zu points.", cloud_msg.data.size());
    }
    ...
    if (vm_.elev_pub->get_subscription_count() >= 1) {
      vec_E<Vec3f> elev_points;
      elev_map_->boxSearch(box_min, box_max, elev_points);
      vecEVec3fToPC2(elev_points, cloud_msg);
      cloud_msg.header.stamp = nh_->get_clock()->now();
      vm_.elev_pub->publish(cloud_msg);
    }
    ...
    if (cfg_.esdf_en) {
      if (vm_.esdf_pub->get_subscription_count() >= 1) {
        PointCloud pc;
        esdf_map_->getPositiveESDFPointCloud(box_min, box_max, robot_state_.p.z(), pc);
      ...
      //     double cur_t = nh_->get_clock()->now().seconds();
      //     if (cur_t - last_print_t > 5.0) {
      //         RCLCPP_WARN(nh_->get_logger(), "ESDF enabled but no subscriber found for topic
      //         'rog_map/esdf'."); last_print_t = cur_t;
      //     }
      // }
    ...
    visualizeBoundingBox(
      mkr_arr, nh_->get_clock()->now().seconds(), box_min, box_max, "Visualization Range", Color::Purple());
    visualizeText(mkr_arr,
      nh_->get_clock()->now().seconds(),
...
public:
  ...
  ROGMapROS(const rclcpp::Node::SharedPtr nh, const std::string & cfg_path) : nh_(nh)
  {
    ...
    const rclcpp::QoS qos(rclcpp::QoS(1).reliable().keep_last(1).durability_volatile());
    ...
    if (cfg_.visualization_en) {
      ...
      vm_.elev_pub = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("rog_map/elevation", qos);
      ...
      if (cfg_.viz_time_rate > 0) {
        ...
        vm_.viz_timer = nh_->create_wall_timer(std::chrono::milliseconds(cbk_dt_ms),
    ...
    if (cfg_.ros_callback_en) {
      ...
      rc_.update_timer = nh_->create_wall_timer(std::chrono::milliseconds(1),  // 0.001秒，即1毫秒
