...
void ProbMap::initProbMap()
{
  ...
  // IMPORTANT: construct sub-maps after virtual heights are discretized,
  // so they all share the same (recomputed) ceil/ground constraints.
  inf_map_ = std::make_shared<InfMap>(cfg_);
  elev_map_ = std::make_shared<ElevationMap>(cfg_);
...
void ProbMap::forceUnknown(const Vec3i & id_g)
{
  ...
  if (isOccupied(ret)) {
    ...
    // 更新膨胀层
    if (inf_map_) {
      inf_map_->updateGridCounter(
        center_pos, super_utils::GridType::OCCUPIED, super_utils::GridType::UNKNOWN);
    }
    // 更新 ESDF 距离场
...
void ProbMap::updateOccPointCloud(const PointCloud & input_cloud)
{
  ...
  std::cout << GREEN << " -- [ROGMapCore] Input cloud size: " << cloud_in_size
            << ", Cache size: " << raycast_data_.update_cache_id_g.size() << " -- " << RESET << std::endl;
  if (cfg_.map_sliding_en) {
...
void ProbMap::updateProbMap(const PointCloud & cloud, const Pose & pose)
{
  ...
  if (raycast_data_.batch_update_counter == 0 && cfg_.map_sliding_en &&
    ...
    slideAllMap(pos);
    std::cout << GREEN
              << " -- [ROGMapCore] Map sliding done. New origin: " << local_map_origin_d_.transpose()
              << " -- " << RESET << std::endl;
  }
...
void ProbMap::raycastProcess(const PointCloud & input_cloud, const Vec3f & cur_odom)
{
  ...
  for (const auto & pcl_p : input_cloud) {
    ...
    // [Map Filter] Relative Height Filter
    if (p.z() > cur_odom.z() + cfg_.relative_ceil_height) {
      update_hit = false;
      // find the intersect point with the relative ceil
      const double dz = p.z() - cur_odom.z();
      const double pc = cfg_.relative_ceil_height;
      p = cur_odom + (p - cur_odom) * (pc / dz);
    }

    // 绝对地面限制 (virtual_ground_height) 可以保留，防止把地底当空闲
    if (p.z() < cfg_.virtual_ground_height) {
      update_hit = false;
