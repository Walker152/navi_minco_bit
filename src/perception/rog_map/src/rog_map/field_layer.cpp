...
void DynamicLayer::updateFromMask(
    ...
    rebuild(width, height, resolution, origin, mask, inflation_radius, max_distance, min_distance,
            clamp_distance, interpolation);
}
...
void DynamicLayer::evaluate(const Eigen::Vector3d &pos, double &dist, Eigen::Vector3d &grad) const {
    ...
    if (interpolation_ == InterpolationMode::QUADRATIC &&
        ix >= 1 && iy >= 1 && ix + 1 < width_ && iy + 1 < height_) {
