...
bool InfMap::isOccupiedInflate(const Vec3i & id_g) const
{
  if (!insideLocalMap(id_g))
    return false;
  if (id_g.z() > cfg_.inf_virtual_ceil_height_id_g)
    return true;
  if (id_g.z() < cfg_.inf_virtual_ground_height_id_g)
    return true;
  ...
  const Vec3f & box_min, const Vec3f & box_max, const GridType & gt, vec_E<Vec3f> & out_points) const
{
  ...
  if (gt == UNKNOWN) {
    if (!cfg_.unk_inflation_en) {
      out_points.clear();
      std::cout << YELLOW << " -- [ROG] Unknown inflation is not enabled, cannot perform box search."
                << RESET << std::endl;
      return;
    }
    ...
    // std::cout << "boxSearch OCCUPIED called" << std::endl;
      ...
      for (int j = box_min_id_g.y(); j <= box_max_id_g.y(); j++) {
        for (int k = box_min_id_g.z(); k <= box_max_id_g.z(); k++) {
          ...
          if (isOccupiedInflate(id_g)) {
            ...
            globalIndexToPos(id_g, pos);
            out_points.push_back(pos);
            // std::cout << "  Occupied point: " << pos.transpose() << std::endl;
          }
        }
      }
    }
...
void InfMap::updateUnkInflation(const Vec3i & id_g, const bool is_add)
{
  ...
  for (const auto & nei : cfg_.unk_inf_spherical_neighbor) {
    ...
    if (imd_.unk_inflate_cnt[addr] < 0 || imd_.unk_inflate_cnt[addr] > imd_.unk_neighbor_num) {
      std::cout << "unk_inflate_cnt: " << imd_.unk_inflate_cnt[addr]
                << " unk_neighbor_num: " << imd_.unk_neighbor_num << std::endl;
      throw std::runtime_error(" -- [IM] Negative occupancy counter, which should not happened.!");
    }
#endif
  }
...
void InfMap::triggerJumpingEdge(
  const rog_map::Vec3i & id_g, const rog_map::GridType & from_type, const rog_map::GridType & to_type)
{
