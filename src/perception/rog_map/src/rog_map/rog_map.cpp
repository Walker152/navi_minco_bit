...
void ROGMap::init() {
    ...
    if (cfg_.map_sliding_en) {
        ...
        // Elevation map is created in ProbMap::initProbMap; initialize 2.5D buffer and align sliding
        elev_map_->initElevationMap(cfg_.half_map_size_i, cfg_.resolution, cfg_.map_sliding_en, cfg_.map_sliding_thresh, cfg_.fix_map_origin);
        elev_map_->mapSliding(cfg_.fix_map_origin);
    }
    else {
    ...
    if (cfg_.load_pcd_en) {
        ...
        slideAllMap(robot_state_.p);

        if(cfg_.esdf_en) {
            esdf_map_->updateESDF3D(robot_state_.p);
        }
        // Initial elevation compute for the visible range
        if (elev_map_) {
...
void ROGMap::updateMap(const PointCloud& cloud, const Pose& pose) {
    ...
    // Update elevation in a local box around robot after map update
    if (elev_map_) {
