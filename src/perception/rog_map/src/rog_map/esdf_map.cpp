...
void ESDFMap::initESDFMap(const rog_map::Vec3i & half_prob_map_size_i,
  const double & prob_map_resolution,
  const double & temp_counter_map_resolution,
  const rog_map::Vec3f & local_update_box,
  const bool & map_sliding_en,
  const double & sliding_thresh,
  const rog_map::Vec3f & fix_map_origin,
  const double & unk_thresh)
{
  ...
  half_local_update_box_i_ = half_local_update_box_i_.cwiseMax(Vec3i::Ones());

  update_local_map_min_i_.setZero();
  update_local_map_max_i_.setZero();
...
void ESDFMap::updateESDF3D(const rog_map::Vec3f & cur_odom)
{
  ...
  // std::cout << "updateESDF3D called with " << cur_odom.transpose() << std::endl;
  using namespace std;
  TimeConsuming up_t("updateESDF3D", false);
  Vec3i id_l, cur_odom_i;
  posToGlobalIndex(cur_odom, cur_odom_i);

  // std::cout << "updateESDF3D: cur_odom=" << cur_odom.transpose()
  //           << ", cur_odom_i=" << cur_odom_i.transpose()
  //           << ", half_box=" << half_local_update_box_i_.transpose()
  //           << ", bound_min=" << local_map_bound_min_i_.transpose()
  //           << ", bound_max=" << local_map_bound_max_i_.transpose() << std::endl;
  ...
  auto getEsdfLocalIndexHash = [&](
                                 const int & id_x, const int & id_y, const int & id_z, bool print = false) {
  ...
  for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
    for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
      fillESDF(
        [&](int z) {
          return isOccupied(
                   getEsdfLocalIndexHash(x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0],
        ...
        [&](int z, double val) {
          tmp_buffer1_[getEsdfLocalIndexHash(x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0],
            y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
            z)] = val;
        },
        min_esdf[2],
  ...
  for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
    for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
      fillESDF(
        ...
        [&](int y, double val) {
          tmp_buffer2_[getEsdfLocalIndexHash(x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0],
            y,
  ...
  for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
    for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
      fillESDF(
        [&](int x) {
          return tmp_buffer2_[getEsdfLocalIndexHash(x,
            y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
            z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])];
        },
        [&](int x, double val) {
          distance_buffer[getEsdfLocalIndexHash(x,
            y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
            z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])] =
  ...
  for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
    for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
      fillESDF(
        ...
        [&](int x, double val) {
          tmp_buffer1_[getEsdfLocalIndexHash(x,
            y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
            z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])] =
            sc_.resolution * std::sqrt(val);
        },
  ...
  for (int x = min_esdf(0); x <= max_esdf(0); ++x)
    for (int y = min_esdf(1); y <= max_esdf(1); ++y)
      ...
      // std::cout << "Start updateESDF3D..." << std::endl;
      // double dt = up_t.stop();
...
void ESDFMap::getPositiveESDFPointCloud(const rog_map::Vec3f & box_min_d,
  ...
  pcl::PointCloud<pcl::PointXYZI> & pcl_pc)
{
  ...
  // Debug print
  // std::cout << "getPositiveESDFPointCloud: box_min_i=" << box_min_i.transpose()
  //           << ", box_max_i=" << box_max_i.transpose()
  //           << ", z=" << z
  //           << ", update_min=" << update_local_map_min_i_.transpose()
  //           << ", update_max=" << update_local_map_max_i_.transpose() << std::endl;

  if (box_min_i.x() > box_max_i.x() || box_min_i.y() > box_max_i.y()) {
    // std::cout << "ESDF PointCloud range empty! box_min_i: " << box_min_i.transpose() << " box_max_i: " <<
    // box_max_i.transpose() << " update_min: " << update_local_map_min_i_.transpose() << " update_max: " <<
    // update_local_map_max_i_.transpose() << std::endl;
  }
...
void ESDFMap::getNegativeESDFPointCloud(const rog_map::Vec3f & box_min_d,
  const rog_map::Vec3f & box_max_d,
  const double & visualize_z,
  pcl::PointCloud<pcl::PointXYZI> & pcl_pc)
{
...
void ESDFMap::fillESDF(F_get_val f_get_val,
  F_set_val f_set_val,
  const int & start,
  const int & end,
  const int & dim,
...
void ESDFMap::interpolateTrilinearEDT(double values[2][2][2], const Eigen::Vector3d & diff, double & value)
{
...
void ESDFMap::interpolateTrilinearFirstGrad(
  double values[2][2][2], const Eigen::Vector3d & diff, Eigen::Vector3d & grad)
{
  // trilinear interpolation
  double v00 = (1 - diff(0)) * values[0][0][0] + diff(0) * values[1][0][0];  // b
  double v01 = (1 - diff(0)) * values[0][0][1] + diff(0) * values[1][0][1];  // d
...
void ESDFMap::interpolateTrilinearSecondGrad(
  double first_grad[2][2][2][3], const Eigen::Vector3d & diff, Eigen::Vector3d & grad)
{
