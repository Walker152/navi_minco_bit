...
namespace rog_map {
    ...
    void ESDFMap::getPositiveESDFPointCloud(const rog_map::Vec3f &box_min_d, const rog_map::Vec3f &box_max_d,
        ...
        if (box_min_i.x() > box_max_i.x() || box_min_i.y() > box_max_i.y()) {
             // std::cout << "ESDF PointCloud range empty! box_min_i: " << box_min_i.transpose() << " box_max_i: " << box_max_i.transpose() << " update_min: " << update_local_map_min_i_.transpose() << " update_max: " << update_local_map_max_i_.transpose() << std::endl;
        }
