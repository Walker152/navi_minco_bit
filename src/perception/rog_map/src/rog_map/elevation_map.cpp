...
void ElevationMap::updateElevation(const Vec3f & pos, float elevation, GridType grid_type)
{
  ...
  // 限制最低高度不能低于虚拟地面高度
  float constrained_elevation = std::max(elevation, static_cast<float>(cfg_.virtual_ground_height));

  // 更新高程值（加权平均）
  int count = emd_.update_counts[addr];
  emd_.elevations[addr] = (emd_.elevations[addr] * count + constrained_elevation) / (count + 1);
  emd_.update_counts[addr] = count + 1;

  // 更新统计信息
  emd_.min_elevation = std::min(emd_.min_elevation, constrained_elevation);
  emd_.max_elevation = std::max(emd_.max_elevation, constrained_elevation);
...
void ElevationMap::updateElevationBatch(
  const std::vector<Vec3f> & points, const std::vector<float> & elevations, GridType grid_type)
{
...
void ElevationMap::getTerrainProfile(
  const Vec3f & start, const Vec3f & end, std::vector<float> & elevations) const
{
  elevations.clear();
...
void ElevationMap::findLocalMinMax(
  const Vec3f & center, float radius, float & min_elev, float & max_elev) const
{
...
float ElevationMap::interpolateElevation(const Vec3f & pos) const
{
  // 双线性插值实现（基于 2.5D 缓冲）
  Vec3i base_id_g;
  elevationMapPosToGlobalIndex(pos, base_id_g);
  Vec3f base_pos;
  elevationMapGlobalIndexToPos(base_id_g, base_pos);
...
void ElevationMap::updateTerrainAnalysis(const Vec3i & id_g)
{
  ...
  std::vector<float> neighbor_elevs;
  getNeighborElevations(id_g, neighbor_elevs);
  ...
  if (addr2d >= 0 && addr2d < static_cast<int>(elevation_xy_.size()))
    center_elev = elevation_xy_[addr2d];

  if (!std::isnan(center_elev) && neighbor_elevs.size() >= 3) {
    float max_diff = 0.0f;
    for (float elev : neighbor_elevs) {
      if (!std::isnan(elev))
        max_diff = std::max(max_diff, std::abs(elev - center_elev));
    }
  ...
  // 计算粗糙度：使用世界坐标
  Vec3f world_pos;
  elevationMapGlobalIndexToPos(id_g, world_pos);
  emd_.roughness_data[addr] = getRoughness(world_pos, 0.2f);
}

void ElevationMap::getNeighborElevations(const Vec3i & id_g, std::vector<float> & neighbor_elevs) const
{
  ...
  for (int dx = -1; dx <= 1; ++dx) {
    for (int dy = -1; dy <= 1; ++dy) {
      ...
      if (insideLocalMap(neighbor_id)) {
        ...
        if (a >= 0 && a < static_cast<int>(elevation_xy_.size()))
          neighbor_elevs.push_back(static_cast<float>(elevation_xy_[a]));
      }
    }
  }
}

void ElevationMap::triggerJumpingEdge(
  const Vec3i & id_g, const GridType & from_type, const GridType & to_type)
{
  // 若网格状态未变化，直接返回
...
void ElevationMap::updateFromMap(const ProbMap & map, const Vec3f & box_min, const Vec3f & box_max)
{
  ...
  if (filter_min_id > filter_max_id)
    std::swap(filter_min_id, filter_max_id);
  if (map_min_id > map_max_id)
    std::swap(map_min_id, map_max_id);
  ...
  for (int x = min_g.x(); x <= max_g.x(); ++x) {
    for (int y = min_g.y(); y <= max_g.y(); ++y) {
      ...
      if (addr >= 0 && addr < static_cast<int>(elevation_xy_.size())) {
        if (!std::isnan(elev)) {
          elev = std::max(elev, cfg_.virtual_ground_height);
        }
