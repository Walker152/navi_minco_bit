...
CellType classifyCell(const ColumnStats &stats, CellData &cell, const ProjectionLayerConfig &config, double resolution) {
    ...
    if (stats.occupied > 0 && std::isfinite(stats.min_z) && std::isfinite(stats.max_z)) {
        ...
        cell.ratio = static_cast<float>(
            (static_cast<double>(stats.occupied + 1) * resolution) /
    ...
    if (config.terrain_enable) {
        ...
        const bool surface_ok = !has_ground ||
            (static_cast<double>(cell.height) <= std::max(config.surface_thickness, resolution) ||
        ...
        if (body_band_blocked) {
            const double body_band_height = std::isfinite(stats.body_band_min_z) && std::isfinite(stats.body_band_max_z)
                ? stats.body_band_max_z - stats.body_band_min_z
                : static_cast<double>(cell.height);
            const bool crossable_body_band =
                body_band_height <= std::max(config.surface_thickness, resolution) &&
...
void ProjectionLayer::updateFull(
    ...
    const bool geometry_changed = width_ != width || height_ != height || std::abs(resolution_ - resolution) > 1.0e-9 ||
...
void ProjectionLayer::updateDirty(
    ...
    expanded.reserve(dirty_columns.size() * static_cast<size_t>((2 * dirty_radius + 1) * (2 * dirty_radius + 1)));
...
bool ProjectionLayer::matchesGeometry(
    ...
    return width_ == width && height_ == height && cells_.size() == static_cast<size_t>(width) * static_cast<size_t>(height) &&
...
CellType ProjectionLayer::applyHysteresis(size_t idx, CellType raw_type, const ProjectionLayerConfig &config) {
    ...
    if (!config.hysteresis_en || config.hysteresis_count <= 0 || cell.type == CellType::UNKNOWN ||
        ...
        cell.stable_count = raw_type == cell.type ? static_cast<uint8_t>(std::min<int>(255, cell.stable_count + 1)) : 0U;
        return cell.type;
    }
...
void ProjectionLayer::updateTerrainNeighborhood(
    const ProjectionLayerConfig &config,
    const std::vector<uint8_t> *update_mask) {
    ...
    for (int y = 0; y < height_; ++y) {
        for (int x = 0; x < width_; ++x) {
            ...
            if (std::isfinite(cell.ground_z)) {
                for (int dy = -1; dy <= 1; ++dy) {
                    for (int dx = -1; dx <= 1; ++dx) {
                        ...
                        const size_t nidx = static_cast<size_t>(ny) * static_cast<size_t>(width_) + static_cast<size_t>(nx);
                        const auto &neighbor = before[nidx];
                        if (!std::isfinite(neighbor.ground_z)) {
                            continue;
                        }
                        const double dz = std::abs(static_cast<double>(cell.ground_z) - static_cast<double>(neighbor.ground_z));
            ...
            if (cell.type == CellType::PASSABLE && (max_step > config.max_step_height || max_slope > config.max_slope_deg)) {
...
void ProjectionLayer::applyHoleFill(const ProjectionLayerConfig &config, const std::vector<uint8_t> *update_mask) {
    if (!config.hole_fill_en || config.hole_fill_radius <= 0 || config.hole_fill_min_occupied_neighbors <= 0 ||
        width_ <= 0 || height_ <= 0 || cells_.empty()) {
        return;
    }
