  ...
  std::vector<Detected_Obj> & obj_list) const
{
  ...
  constexpr int kUnclassified = 0;
  constexpr int kNoise = -1;
  const int min_pts = std::max(1, config_.dbscan_min_points);

  std::vector<bool> visited(cloud_2d->size(), false);
  ...
  for (size_t i = 0; i < cloud_2d->size(); ++i) {
    ...
    region_query(static_cast<int>(i), neighbors);
    ...
    ++cluster_id;
    std::deque<int> seed_queue;
    std::vector<bool> in_queue(cloud_2d->size(), false);
    for (const int idx : neighbors) {
      if (!in_queue[static_cast<size_t>(idx)]) {
        seed_queue.push_back(idx);
        in_queue[static_cast<size_t>(idx)] = true;
      }
      labels[static_cast<size_t>(idx)] = cluster_id;
    }

    while (!seed_queue.empty()) {
      const int current_idx = seed_queue.front();
      seed_queue.pop_front();

      if (!visited[static_cast<size_t>(current_idx)]) {
        visited[static_cast<size_t>(current_idx)] = true;
        std::vector<int> current_neighbors;
        region_query(current_idx, current_neighbors);
        if (static_cast<int>(current_neighbors.size()) >= min_pts) {
          for (const int n_idx : current_neighbors) {
            if (!in_queue[static_cast<size_t>(n_idx)]) {
              seed_queue.push_back(n_idx);
              in_queue[static_cast<size_t>(n_idx)] = true;
            }
          }
        }
      }
  ...
  std::unordered_map<int, std::vector<int>> clustered_indices;
  clustered_indices.reserve(static_cast<size_t>(cluster_id));
  for (size_t i = 0; i < labels.size(); ++i) {
    const int label = labels[i];
    if (label > 0) {
      clustered_indices[label].push_back(static_cast<int>(i));
    }
  }
  ...
  for (const auto & cluster_kv : clustered_indices) {
    ...
    cluster->reserve(cluster_idx.size());
