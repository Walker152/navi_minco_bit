...
class FdManager
{
 ...
 public:
  ...
  void add(const std::string& name, int fd, fd_read_cb readcb,
           ...
           bufferevent_event_cb eventcb = nullptr, void* cbarg = nullptr)
  {
    // TODO 修改传入参数
    //检查fd是否重复插入
    ...
    shared_bev bev(bufferevent_socket_new(
 ...
 private:
  ...
  {
    printf(
        "Caught an interrupt signal; exiting cleanly in two "
        "seconds.\n");
