...
class FdWrapper
{
    ...
    struct TimerData
    {
      ...
      public:
        ...
        explicit TimerData(
            ...
            : m_timer_manager( timer_manager ), m_timer( timer )
        {
        }
        ~TimerData( )
        {
  ...
  private:
    FdWrapper( const std::string &name, shared_bev bev, fd_read_cb read_cb )
        : m_timer_manager( ), m_bev( bev ), m_seqnum2data( ), m_rtt( 100 ),
          m_rto( 2 * m_rtt ), m_is_activate( true ), m_name( name ),
          m_read_cb( read_cb )
    {
        // TODO this指针的安全性
        std::thread t( [ this ]( ) { this->m_timer_manager.run( ); } );
  ...
  private:
    ...
    inline void ack( const uint16_t seq_num )
    {
        m_seqnum2data.erase( seq_num );
    }
    ...
    inline void insert( const uint16_t seq_num, const ByteArray &data )
    {
        ...
        std::weak_ptr< MyUtils::MyTimer::__detail::Timer > timer =
            this->m_timer_manager.addTimer(
                m_rto, false, [ bev, data, this ]( ) {
                    if ( bufferevent_write( bev.get( ), data.get( ),
                                            data.size( ) ) == -1 )
                    {
        ...
        m_seqnum2data.insert(
            std::make_pair( seq_num, TimerData( &m_timer_manager, timer ) ) );
    }

    inline void erase( const uint16_t seq_num )
    {
        this->m_seqnum2data.erase( seq_num );
    }
};
...
class FdManager
{
  ...
  private:
    ...
    MyUtils::DataType::BiMap< int, FdWrapper * >
        m_fd2wrapper; //句柄到 FdWrapper 的映射
  ...
  public:
     ...
     * @return bufferevent *
     */
    void add( const std::string &name, int fd, fd_read_cb readcb,
              ...
              bufferevent_event_cb eventcb = NULL, void *cbarg = NULL )
    {
        ...
        shared_bev bev( bufferevent_socket_new( m_base, fd,
                                                BEV_OPT_CLOSE_ON_FREE |
                                                    BEV_OPT_THREADSAFE ),
                        []( bufferevent *bev ) { bufferevent_free( bev ); } );

        bufferevent_setcb( bev.get( ), __read_cb, NULL, __event_cb,
                           ( void * ) this );
  ...
  public:
              ...
              const size_t &data_len ) noexcept
    {
        ...
        if ( it != this->m_fd2name.end( ) )
        {
            ...
            return bufferevent_write( it2->second->bev( ).get( ), data,
                                      data_len );
        }
        else
        {
  ...
  private:
    ...
    static void __read_cb( struct bufferevent *bev, void *arg )
    {
        ...
        FdManager *ths = ( FdManager * ) arg;
        auto it = ths->m_fd2wrapper.find_by_first(
