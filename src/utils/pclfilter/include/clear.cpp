...
namespace pclfilter
{
    ...
    void clear::init_basemap()
    {
        cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            "/cloud_registered", 10,
            std::bind(&clear::basemapCloudCB, this, std::placeholders::_1));
    ...
    void clear::basemapCloudCB(const sensor_msgs::msg::PointCloud2::SharedPtr input_msg)
    {
        ...
        for (size_t i = 0; i < min_pts_.size(); ++i)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Filter %zu: from %zu to %zu points (removed %zu)", i, before_size, after_size, before_size - after_size);
        }
    ...
    void clear::init_odom()
    {
        basemap_cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            "/cloud_registered", 10,
            std::bind(&clear::cloudCB, this, std::placeholders::_1));
        cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_filter_baselink", 10);
    ...
    void clear::init_mapwithodom()
    {
        ...
        cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            "/cloud_registered", 10,
            std::bind(&clear::combineCloudCB, this, std::placeholders::_1));
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/odom", 10,
            std::bind(&clear::odomCB, this, std::placeholders::_1));
        cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_filter_baselink", 10);
    ...
    void clear::cloudCB(const sensor_msgs::msg::PointCloud2::SharedPtr input_msg)
    {
        ...
        else
        {
            try
            {
                ...
                tf2::doTransform(*input_msg, cloud_in_base_link,
    ...
    bool clear::loadparam(const std::string &param_name)
    {
        ...
        try
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Loaded %zu polygons from %s", 
                polygons_.size(), yaml_file.c_str());
            return true;
        }
    ...
    bool clear::loadcubeParam(const std::string &param_name)
    {
        ...
        try
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Loaded %zu cuboids from %s", 
                min_pts_.size(), yaml_file.c_str());
            return true;
        }
        catch (const YAML::Exception& e)
        {
