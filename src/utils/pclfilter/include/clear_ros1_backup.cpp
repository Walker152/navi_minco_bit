...
{
  cloud_sub_ =
    nh_.subscribe<sensor_msgs::PointCloud2>("/cloud_registered", 1, &clear::basemapCloudCB, this);
...
void clear::basemapCloudCB(const sensor_msgs::PointCloud2ConstPtr & input_msg)
{
  ...
  try {
    if (!tf_buffer.canTransform(target_frame, source_frame, input_msg->header.stamp, ros::Duration(0.1))) {
      ROS_WARN("Cannot transform from %s to %s at time %u.%u",
        source_frame.c_str(),
        target_frame.c_str(),
        input_msg->header.stamp.sec,
        input_msg->header.stamp.nsec);
      return;
    }
...
{
  basemap_cloud_sub_ =
    nh_.subscribe<sensor_msgs::PointCloud2>("/cloud_registered", 1, &clear::cloudCB, this);
...
void clear::init_mapwithodom()
{
  if_clear_ = false;
  cloud_sub_ =
    nh_.subscribe<sensor_msgs::PointCloud2>("/cloud_registered", 1, &clear::combineCloudCB, this);
  odom_sub_ = nh_.subscribe<nav_msgs::Odometry>("/odom_topic", 1, &clear::odomCB, this);
  cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/cloud_filter_baselink", 1);
...
bool clear::loadcubeParam(const std::string & param_name)
{
  ...
  for (int i = 0; i < cuboid_list.size(); ++i) {
    ...
    if (min_val.getType() != XmlRpc::XmlRpcValue::TypeArray ||
        max_val.getType() != XmlRpc::XmlRpcValue::TypeArray || min_val.size() != 4 || max_val.size() != 4) {
