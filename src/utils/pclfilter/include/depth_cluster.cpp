...
namespace pclfilter {
DepthCluster::DepthCluster(float vertcal_resolution,
...
: vertcal_resolution_(vertcal_resolution), horizontal_resolution_(horizontal_resolution),
  lidar_lines_(lidar_lines), min_cluster_size_(min_cluster_size), normal_estimation_k_(normal_k),
  depth_threshold_(depth_threshold), use_euclidean_(use_euclidean),
  euclidean_tolerance_(euclidean_tolerance), euclidean_min_cluster_size_(euclidean_min_size),
  euclidean_max_cluster_size_(euclidean_max_size), normal_curvature_threshold_(normal_curvature_threshold),
  auto_estimate_reference_normal_(auto_estimate_reference_normal),
  use_adaptive_radius_(use_adaptive_radius), use_temporal_filter_(use_temporal_filter),
  ...
  double current_time)
{
  ...
  cout << "Temporal filter: raw ground=" << current_ground.size() << ", filtered=" << filtered_ground.size()
       << endl;
...
void DepthCluster::setInputCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr & msg)
{
  ...
  if (use_euclidean_) {
    ...
    for (int idx : ground_points_indices_)
      is_ground[idx] = true;
  ...
  const vector<vector<PointInfo>> & depth_image)
{
  ...
  while (!q.empty()) {
    ...
    for (int k = 0; k < 8; ++k) {
      int nr = r + dr[k];
      int nc = c + dc[k];
      if (nr < 0 || nr >= image_rows_ || nc < 0 || nc >= image_cols_)
        continue;
      if (pixel_ground[nr][nc])
        continue;
      if (depth_image[nr][nc].indices.empty())
        continue;

      float neigh_depth = depth_image[nr][nc].depth_;
      if (fabs(center_depth - neigh_depth) > depth_threshold_)
        continue;
...
vector<pcl::PointIndices> DepthCluster::euclideanClustering(
...
vector<vector<PointInfo>> DepthCluster::generateDepthImage(
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_fused_ptr)
{
  ...
  for (int r = 0; r < rows; ++r) {
    for (int c = 0; c < cols; ++c) {
      if (label_image[r][c] == -1 && !depth_image[r][c].indices.empty()) {
        ...
        cluster_indices.insert(
          cluster_indices.end(), depth_image[r][c].indices.begin(), depth_image[r][c].indices.end());
        ...
        while (!q.empty()) {
          ...
          for (auto [dr, dc] : neighbor) {
            ...
            nr = np.first;
            nc = np.second;
            if (label_image[nr][nc] != -1 || depth_image[nr][nc].indices.empty())
              continue;
            float diff = fabs(cur_depth - depth_image[nr][nc].depth_);
            if (diff >= depth_threshold_)
              continue;
            q.push({nr, nc});
            label_image[nr][nc] = label_val;
            cluster_indices.insert(cluster_indices.end(),
              depth_image[nr][nc].indices.begin(),
              depth_image[nr][nc].indices.end());
          }
        }
        label_val++;
...
bool DepthCluster::judgmentCondition(const vector<vector<PointInfo>> & depth_image,
  ...
  const pair<int, int> & neigh)
{
  float diff =
    fabs(depth_image[target.first][target.second].depth_ - depth_image[neigh.first][neigh.second].depth_);
