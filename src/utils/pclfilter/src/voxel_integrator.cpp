  ...
  VoxelIntegrator() : Node("voxel_integrator")
  {
    ...
    if (use_ground_) {
      ground_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/ground_points_map", 10, std::bind(&VoxelIntegrator::groundCallback, this, std::placeholders::_1));
    }
    if (use_obstacles_) {
      obstacle_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>("/obstacle_clusters_map",
        10,
    ...
    if (publish_freq_ > 0) {
      timer_ = this->create_wall_timer(
        std::chrono::duration<double>(1.0 / publish_freq_), std::bind(&VoxelIntegrator::publishMap, this));
    }
  }
